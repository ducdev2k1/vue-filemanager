...
<template>
  <div ref="wrapperRef" class="c-data-table-virtual_wrapper">
    <template v-if="selectedItems.length > 0">
      <div class="c-data-table-virtual_header-selected" :style="{ minHeight: `${rowHeight}px` }">
        <v-checkbox
          v-if="showCheckbox"
          hide-details
    ...
    <div ref="tableRef" class="c-data-table-virtual noselect relative">
      <!-- Table header -->
      <div class="c-data-table-virtual__header">
        <table class="w-full">
          <thead>
            <tr>
              ...
              <th v-for="header in headerTable" :key="header.key" :style="header.width ? { width: header.width } : {}">
                {{ header.title }}
              </th>
            </tr>
          </thead>
      ...
      <div
        ref="tableBodyRef"
        class="c-data-table-virtual__body"
        :style="{ height: heightTable }"
        @scroll="handleScroll">
        <!-- Spacer for virtual scrolling -->
        <div class="c-data-table-virtual__spacer" :style="{ height: `${totalHeight}px` }"></div>

        <!-- Visible rows -->
        <div class="c-data-table-virtual__visible-rows" :style="{ transform: `translateY(${offsetY}px)` }">
          <template v-for="(item, localIndex) in visibleItems" :key="item.key">
            <div
              class="c-data-table-virtual__row"
              :class="{
                'c-data-table-virtual__hovered': hoveredRowIndex === startIndex + localIndex,
              ...
              <!-- Checkbox -->
              <div v-if="showCheckbox" class="c-data-table-virtual__cell text-center" style="width: 50px">
                <v-checkbox
                  hide-details
              ...
              <template v-for="header in headerTable" :key="header.key">
                <div class="c-data-table-virtual__cell" :style="header.width ? { width: header.width } : {}">
                  <slot
        ...
        <!-- No data content -->
        <div v-if="dataTable.length === 0 && $slots['no-data.table']" class="c-data-table-virtual__no-data">
...
<style scoped>
  ...
  .c-data-table-virtual_header-selected {
    display: flex;
    align-items: center;
    padding: 0 12px;
  ...
  .c-data-table-virtual__body {
    flex: 1;
    overflow-y: auto;
    position: relative;
  }

  .c-data-table-virtual__spacer {
    position: absolute;
    top: 0;
    left: 0;
    width: 100%;
  ...
  .c-data-table-virtual__visible-rows {
    position: absolute;
    width: 100%;
    left: 0;
  }

  .c-data-table-virtual__row {
    display: flex;
    align-items: center;
    border-bottom: 1px solid var(--v-border-color, #e0e0e0);
  ...
  .c-data-table-virtual__selected-one {
    background-color: var(--v-primary-lighten4, #bbdefb);
  } */

  .c-data-table-virtual__no-data {
    display: flex;
